...
def encodeImage(oa, ob, xmap=None, margins=(1, 1), alpha=None):
    ...
    # check if the watermark does not exceed the allowed size
    if nb.shape[0] > na.shape[0] // 2 - margins[0] * 2 or nb.shape[1] > na.shape[1] - margins[1] * 2:
        raise ValueError("Watermark image is too large to embed into the original image.")
...
def encodeText(oa, text, *args, **kwargs):
    ...
    # except:
    #     font = ImageFont.load_default(oa.shape[0] // 7)

    
    font = ImageFont.truetype("consola.ttf", oa.shape[0] // 7) 
    # #"font = ImageFont.truetype("consola.ttf", oa.shape[0] // 7)" means the font size is 1/7 of the image height
...
if __name__ == "__main__" or True:
    ...
    #python .\Fdsig.py -o .\result\DFT_images\lenna_encoded.jpg  -t SA24221066 -a 10 -v .\Original_images\lenna.png
    ...
    if args.decode:
        ...
        #embed the text
            ...
            #ea: encoded image, fa: frequency domain
